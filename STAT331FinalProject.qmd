...
tidy(energy_emissions_model_clean)

```

Simple regression equation:

$$ \hat{y}(Average\,CO2\,Emission) = 693309.6 - 17770.2 * Percent\,Consumption\,Renewable $$
...
```{r}
energy_emissions_model_clean |>
  augment() |>
  summarize(`Variance of Fitted` = var(.fitted),
            `Variance of Residuals` = var(.resid),
            `Variance of Total_CO2_Emissions` = var(avg_emissions)) |>
