...
raw_graph <- master |> 
  ...
  geom_smooth(method = "lm", color = 'lightgoldenrod4', size = 0.5) +
...
residuals <- energy_emissions_model |>
  augment() |>
  rename(Residual = .resid,
         Fitted = .fitted) |>
  mutate(`Residual Sign` = ifelse(Residual > 0 , 1, 0))
...
$R^2$ is 0.803297, which means the simulated data can explain around 80% variation of the observed data. In this case, we suppose that the one time simulation till now did a moderately good job.
